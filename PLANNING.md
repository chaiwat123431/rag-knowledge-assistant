...
| Embeddings | `all-MiniLM-L6-v2`, local, via `chromadb`'s `ONNXMiniLM_L6_V2` (ONNX Runtime) | Runs locally, no API cost or key, good baseline quality for a single-user portfolio project. Was OpenAI `text-embedding-3-small`; switched to avoid paid API dependency. Was `sentence-transformers` (PyTorch); switched to the same model's ONNX export (already a `chromadb` dependency, no new package) after PyTorch's ~500MB runtime import alone exceeded Render's 512MB Starter plan — see Progress. |
...
- [x] Reduce backend memory footprint for Render's 512MB Starter plan (`embeddings.py` + `main.py` + tests) — `feature/reduce-memory-footprint`
  - **Root cause, measured**: the backend was crashing (OOM) mid-request on Render, not at startup — `/health` worked, `/documents` and `/query` didn't. Confirmed the embedding model and Chroma client were already lazily loaded (double-checked locking, since PR #3) — the problem wasn't *when* they loaded, it was *how much* they cost once they did. Isolated measurements (`/usr/bin/time -l`, real add-document-then-query cycle): the full app import alone is ~82MB; `sentence-transformers` (PyTorch) importing and loading `all-MiniLM-L6-v2` costs **~505-513MB by itself** — over the entire 512MB budget before FastAPI, Chroma, or the request's own work are even added. That cost comes from the PyTorch runtime import (~400MB), not the ~90MB model weights: a smaller sentence-transformers model would not have helped.
  - `OMP_NUM_THREADS=1` / `TOKENIZERS_PARALLELISM=false` (`main.py`, `os.environ.setdefault`, set before any other import): measured 8 runs with vs. 8 without, both before and after the runtime swap below — **no reliable effect on peak memory** (run-to-run variance, ~20MB, dwarfs any difference). Kept anyway: real value for avoiding CPU thread contention on a low-vCPU instance under concurrent requests, just not a fix for the OOM itself. An initial 3-run comparison looked like a ~10% win; expanding to 8 runs each showed that was noise, not a real effect — a reminder to size a sample before trusting it, on this project as much as anywhere else.
  - **Fix**: `embeddings.py` now loads the model via `chromadb.utils.embedding_functions.ONNXMiniLM_L6_V2` — the *same* `all-MiniLM-L6-v2` weights, exported to ONNX and run via ONNX Runtime instead of PyTorch — rather than `sentence-transformers`. No new dependency: `onnxruntime` and `tokenizers` already ship as unconditional (non-optional) transitive dependencies of `chromadb`, which the project already depends on. Measured: the same realistic cycle now peaks at **~305-354MB**, comfortably under budget. `sentence-transformers` dropped from `requirements.txt`.
  - **Verified, not assumed, that this is a runtime swap and not a different or degraded model**: embedded a sample of the project's own test texts (short/long, related/unrelated pairs) with both runtimes side by side — max absolute difference between the two runtimes' output vectors was **0.000000** (float32-identical), cosine similarity 1.0. Re-ran the full backend suite unchanged, including every `@pytest.mark.model` test that exercises `MIN_RELEVANCE_SCORE` / `MAX_AUGMENTED_SCORE_DROP_RATIO` / `MIN_CROSS_SOURCE_RATIO` against real embeddings (the exact regression tests for Bugs 1-4 above) — all 21 passed with zero changes. No threshold recalibration was needed; this ONNX export is numerically equivalent to the PyTorch model, not an approximation of it.
  - A different truncation-detection mechanism was needed for the new runtime: sentence-transformers' tokenizer exposed raw (untruncated) token counts directly; the ONNX ef's `tokenizers.Tokenizer` always pads/truncates to 256 internally, so `_warn_on_truncation` now checks each `Encoding.overflowing` (non-empty exactly when truncation happened) instead of comparing a token count. Verified empirically, not just read from source: an over-limit input does not raise (unlike a misreading of the library's own truncation-guard code briefly suggested) — the tokenizer's `enable_truncation` runs first, silently, same net behavior as the old sentence-transformers path.
  - **A measurement error caught and corrected mid-investigation, disclosed rather than quietly fixed**: an early comparison of "call the ONNX embedding function directly" vs. "let Chroma's collection call it internally" showed a large (~170MB) gap that would have driven the architecture choice. The lower number turned out to be from a script that crashed before doing any embedding work at all (an unrelated bug — a 2-character Chroma collection name, below its 3-character minimum — silently combined with `/usr/bin/time` still reporting stats for the failed process). Re-run correctly, both approaches cost the same (~305-354MB); there was no real architectural difference to choose between.
  - `/code-review` fixes, each verified by reproducing the failure first, not just reasoning about it: (1) `_get_model()`'s double-checked locking only built the cheap `ONNXMiniLM_L6_V2()` Python object inside the lock — unlike the old `SentenceTransformer(...)` it replaced, the *expensive* part (download, tokenizer parse, ONNX session build) was deferred to first use, outside the lock. `_warn_on_truncation` accessing `model.tokenizer` before any embed call ever ran crashed outright on a cold cache (reproduced: `Exception: No such file or directory`) — the exact fresh-deploy scenario this whole change targets. Worse, two threads racing in on a cold cache (FastAPI's sync routes run in a threadpool) could both call the shared model object concurrently outside the lock, racing on the same download/extract path (reproduced: an `InvalidProtobuf` error from a torn concurrent write). One fix resolved both: force a throwaway embed inside the lock, so the model is fully warm before any caller — including the one that built it — can ever see it. (2) `os.environ.setdefault(...)` ran before `load_dotenv()` in `main.py`, so `load_dotenv()`'s default `override=False` would silently ignore a value set in `.env` for these two keys specifically — reordered. Verified this exact scenario in isolation (a minimal repro does show the bug), but also verified it was not actually reachable end-to-end through this app as it stood: `app.retrieval.gemini` independently calls `load_dotenv()` too, so `.env` was already loaded as a side effect of importing `app.api.routes`, before either ordering of `main.py`'s own two lines mattered. Fixed anyway — `main.py`'s correctness shouldn't depend on an unrelated module's side effect — and disclosed precisely rather than overclaiming a live production bug that measurement didn't support.
  - `/code-review`, 2nd round: (1) `ingest_document`'s `except OSError` stopped catching the embedding-model-download failure mode after the ONNX switch — the old `sentence-transformers`/`huggingface_hub` path raised `requests.exceptions.ConnectionError` (an `OSError` subclass), the new `httpx`-based one raises `httpx.ConnectError`/`ConnectTimeout` (confirmed, empirically, *not* `OSError` subclasses) — so a network hiccup during a fresh deploy's first model download would 500 bare instead of the intended 503. Fixed by also catching `httpx.HTTPError`, verified with the exact repro (a store whose `add_documents` raises `httpx.ConnectError` now correctly gets mapped to 503). (2) The new concurrency regression test was itself destructive: it `shutil.rmtree`'d `ONNXMiniLM_L6_V2.DOWNLOAD_PATH`, the real, shared, machine-wide model cache (`~/.cache/chroma/onnx_models/...`), not a test-isolated path — confirmed live in this session that running it deleted and forced a redundant re-download of the real cached model. Fixed by monkeypatching that class attribute to a `tmp_path` instead, forcing the same genuinely-cold-cache race without touching the real cache (verified: the real model file's mtime is now unchanged by the test run). (3) The truncation-warning log message lost the exact over-limit token count the old version reported. Investigated two ways to recover it and rejected both after they produced *wrong* numbers under direct measurement, not just seeming fragile: summing `Encoding.overflowing`'s pieces plateaus at a fixed total regardless of true input length (400-word and 1000-word inputs reported the same count); a second, supposedly non-truncating `Tokenizer` loaded from the same `tokenizer.json` came back with its own baked-in limit anyway (reported the same wrong length for a 4-word sentence as for a 1000-word one). Left unrecovered — the actionable part of the warning (which chunk, that it was truncated) still logs correctly, just without a specific count. (4) Trimmed a chunk of duplicated `OMP_NUM_THREADS`/`TOKENIZERS_PARALLELISM` rationale out of `main.py`'s comment in favor of a one-line pointer to `.env.example`, the canonical explanation.
  - `/code-review`, 3rd round: the round-2 `httpx.HTTPError` fix only landed on `/documents`; `/query` had the identical gap (it embeds the question via `VectorStore.query` on every call, so a cold-start download hiccup is at least as reachable there) — reproduced live (a store whose `.query()` raises `httpx.ConnectError`, posted to `/query`, returned a bare 500) and fixed the same way. Module docstring's 503 bullet updated to note this applies to both endpoints, not just ingestion.
